...
func (sl *ServiceSelfLink) Parse(selflink string) error {
	...
	if len(parts) < 2 {
		sl.parent = SelfLinkParent{
			Kind:     KindNamespace,
			SelfLink: NewNamespaceSelfLink(DefaultNamespace),
		}
		sl.name = parts[0]
		return nil
	}
...
func (sl *DeploymentSelfLink) Parse(selflink string) error {
	...
	if len(parts) == 2 {
		...
		sl.parent.SelfLink = NewServiceSelfLink(DefaultNamespace, parts[0])
		sl.name = parts[1]
		return nil
	}
...
func (sl *TaskSelfLink) Parse(selflink string) error {
	...
	if len(parts) == 2 {
		sl.namespace = NewNamespaceSelfLink(DefaultNamespace)
		sl.parent.SelfLink = NewJobSelfLink(DefaultNamespace, parts[0])
		sl.name = parts[1]
		return nil
	}
